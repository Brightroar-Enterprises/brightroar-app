...
class _WalletsScreenState extends State<WalletsScreen>
  ...
  Future<void> _syncBinanceWallets() async {
    if (!mounted) return;
    setState(() => _syncing = true);
    try {
      final result = await ApiClient.syncBinanceWallets();
      final count = result['count'] ?? 0;
      if (!mounted) return;
      _showSnack('Synced $count Binance wallets', AppTheme.positive);
      await _loadData();
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _syncOnchain(Map<String, dynamic> wallet) async {
    if (!mounted) return;
    final walletId = wallet['id']?.toString() ?? '';
    final name = wallet['name']?.toString() ?? '';
    try {
      _showSnack('Syncing $name from blockchain...', AppTheme.primary);
      final result = await ApiClient.syncWalletOnchain(walletId);
      final newBal = result['new_balance'] ?? '0';
      final usd = result['balance_usd'] ?? '0';
      if (!mounted) return;
      _showSnack('$name synced: $newBal ${wallet['asset_symbol']} (\$$usd)', AppTheme.positive);
      await _loadData();
    } catch (e) {
      if (!mounted) return;
      _showSnack(e.toString(), AppTheme.negative);
  ...
  // ── Wallet categorisation ─────────────────────────────────────────────────
  // Case-insensitive comparison — backend returns 'COLD_STORAGE', 'cold_storage', etc.
  bool _isColdStorage(Map w) =>
      w['wallet_type']?.toString().toLowerCase() == 'cold_storage';

  bool _isBinance(Map w) =>
      w['exchange_name']?.toString() == 'Binance';

  List<dynamic> get _internalWallets {
    final wallets = (_walletData?['wallets'] as List?) ?? [];
    return wallets.where((w) => !_isBinance(w) && !_isColdStorage(w)).toList();
  }

  List<dynamic> get _binanceWallets {
    final wallets = (_walletData?['wallets'] as List?) ?? [];
    return wallets.where((w) => _isBinance(w)).toList();
  }

  List<dynamic> get _coldStorageWallets {
    final wallets = (_walletData?['wallets'] as List?) ?? [];
    return wallets.where((w) => _isColdStorage(w) && !_isBinance(w)).toList();
  }
          ...
          if (_loading)
            ...
            SliverPadding(
              ...
              sliver: SliverList(delegate: SliverChildListDelegate([
                ...
                _tabContent(),
                const SizedBox(height: 20),
                if (_binanceData != null) _binanceLiveCard(),
                const SizedBox(height: 20),
              ])),
            ),
        ]),
  ...
  Widget _appBar() => Padding(
    ...
    child: Row(children: [
      ...
      const Spacer(),
      if (_binanceData != null)
        GestureDetector(
          onTap: _syncing ? null : _syncBinanceWallets,
          child: Container(
            ...
            child: _syncing
                ...
                : Row(children: const [
                    ...
                    Text('Sync', style: TextStyle(
                        color: Color(0xFFF0B90B), fontSize: 12, fontWeight: FontWeight.w600)),
                  ]),
          ),
        ),
      GestureDetector(
  ...
  Widget _tabContent() => AnimatedBuilder(
    animation: _tabController,
    builder: (context, _) {
      switch (_tabController.index) {
        case 0: return _walletListSection('Internal Wallets', _internalWallets);
        case 1: return _binanceWalletsList();
        case 2: return _walletListSection('Cold Storage', _coldStorageWallets);
        default: return _walletListSection('Internal Wallets', _internalWallets);
      }
    },
  );
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 32, height: 32,
              decoration: BoxDecoration(color: AppTheme.usdtColor.withOpacity(0.2),
                  borderRadius: BorderRadius.circular(8)),
              child: const Center(child: Text('T',
                  style: TextStyle(color: AppTheme.usdtColor, fontWeight: FontWeight.w700, fontSize: 16)))),
          const SizedBox(width: 10),
          const Expanded(child: Text('Total Portfolio',
        ...
        Row(children: [
          _countBadge('${_internalWallets.length} Internal', AppTheme.primary),
          const SizedBox(width: 8),
          _countBadge('${_binanceWallets.length} Binance', const Color(0xFFF0B90B)),
          const SizedBox(width: 8),
          _countBadge('${_coldStorageWallets.length} Cold', AppTheme.ethColor),
        ]),
        const SizedBox(height: 14),
        Row(children: [
  ...
  // ── Binance wallets tab ────────────────────────────────────────────────────
  Widget _binanceWalletsList() {
    final wallets = _binanceWallets;
    if (wallets.isEmpty) {
      return GlassCard(child: Padding(
        ...
        child: Column(children: [
          Container(width: 56, height: 56,
              decoration: BoxDecoration(
                  color: const Color(0xFFF0B90B).withOpacity(0.1), shape: BoxShape.circle),
              child: const Center(child: Text('B',
                  style: TextStyle(color: Color(0xFFF0B90B), fontWeight: FontWeight.w700, fontSize: 24)))),
          const SizedBox(height: 14),
          const Text('No Binance Wallets', style: TextStyle(
              color: AppTheme.textPrimary, fontSize: 15, fontWeight: FontWeight.w600)),
          const SizedBox(height: 6),
          const Text('Connect your Binance API keys in Settings to auto-create wallets for BTC, ETH, USDT, SOL and DOT.',
              style: TextStyle(color: AppTheme.textSecondary, fontSize: 13, height: 1.5),
              textAlign: TextAlign.center),
          const SizedBox(height: 16),
          ElevatedButton.icon(
            onPressed: _syncBinanceWallets,
            icon: const Icon(Icons.sync, size: 16, color: AppTheme.background),
            label: const Text('Sync Now', style: TextStyle(color: AppTheme.background, fontWeight: FontWeight.w700)),
          ),
        ]),
      ));
    }
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        const Text('Binance Wallets', style: TextStyle(
            ...
            child: Row(children: [
              _syncing
                  ? const SizedBox(width: 12, height: 12,
                      child: CircularProgressIndicator(color: Color(0xFFF0B90B), strokeWidth: 2))
                  : const Icon(Icons.sync, color: Color(0xFFF0B90B), size: 12),
              const SizedBox(width: 4),
              const Text('Sync Balances', style: TextStyle(
                  color: Color(0xFFF0B90B), fontSize: 11, fontWeight: FontWeight.w600)),
            ])),
        ),
      ]),
      const SizedBox(height: 12),
      GlassCard(
          borderColor: const Color(0xFFF0B90B).withOpacity(0.2),
          ...
          child: ListView.separated(
              ...
              itemBuilder: (ctx, i) => _binanceWalletItem(wallets[i]))),
    ]);
  }

  Widget _binanceWalletItem(Map<String, dynamic> w) {
    ...
    final colorMap = {
      ...
      'USDT': AppTheme.usdtColor, 'SOL': AppTheme.solColor, 'DOT': AppTheme.primary,
    };
    final color = colorMap[symbol] ?? AppTheme.primary;
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(w['name'] ?? 'Binance $symbol', style: const TextStyle(
          ...
          Row(children: [
            ...
            const SizedBox(width: 4),
            const Text('Binance · Exchange', style: TextStyle(color: AppTheme.textTertiary, fontSize: 10)),
          ]),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text('${_fmtBalance(balance)} $symbol', style: const TextStyle(
  ...
  // ── Generic wallet list ────────────────────────────────────────────────────
  Widget _walletListSection(String title, List wallets) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('$title (${wallets.length})', style: const TextStyle(
          color: AppTheme.textPrimary, fontSize: 16, fontWeight: FontWeight.w600)),
      const SizedBox(height: 12),
      wallets.isEmpty
          ? GlassCard(child: Padding(
              ...
              child: Column(children: [
                ...
                const SizedBox(height: 12),
                Text('No $title yet', style: const TextStyle(
                    color: AppTheme.textPrimary, fontSize: 15, fontWeight: FontWeight.w600)),
                const SizedBox(height: 6),
                const Text('Tap + Add Wallet to create one',
  ...
  void _showWalletActions(Map<String, dynamic> wallet) {
    final hasAddress = (wallet['address']?.toString() ?? '').isNotEmpty;
    final asset = wallet['asset_symbol']?.toString() ?? '';
    final supportsOnchain = ['ETH', 'USDT', 'USDC'].contains(asset);
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          if (hasAddress && supportsOnchain) ...[
            _actionTile(Icons.cloud_sync_outlined, 'Sync On-Chain Balance', AppTheme.ethColor, () async {
              Navigator.pop(ctx);
              await _syncOnchain(wallet);
            }),
            const SizedBox(height: 12),
          ],
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: Container(width: 40, height: 4,
              decoration: BoxDecoration(color: AppTheme.border, borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 20),
          const Text('Add New Wallet', style: TextStyle(
          ...
          TextField(controller: addressCtrl,
              ...
              decoration: InputDecoration(hintText: '0x... or bc1...',
                  ...
                  onPressed: () async {
                    ...
                    try {
                      await ApiClient.createWallet(
                        name: nameCtrl.text.trim(), walletType: walletType, assetSymbol: asset,
                        address: addressCtrl.text.trim().isEmpty ? null : addressCtrl.text.trim(),
                      );
                      _showSnack('Wallet created!', AppTheme.positive);
                      _loadData();
                    } catch (e) { _showSnack(e.toString(), AppTheme.negative); }
                  },
                  child: const Text('CREATE WALLET', style: TextStyle(
                      color: AppTheme.background, fontWeight: FontWeight.w700)))),
        ]))));
  }
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: Container(width: 40, height: 4,
              decoration: BoxDecoration(color: AppTheme.border, borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 20),
          const Text('Edit Wallet', style: TextStyle(
              color: AppTheme.textPrimary, fontSize: 18, fontWeight: FontWeight.w700)),
          const SizedBox(height: 20),
          const Text('Wallet Name', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12, fontWeight: FontWeight.w500)),
          const SizedBox(height: 8),
          ...
          Row(children: [
            ...
            Expanded(child: ElevatedButton(
                onPressed: () async {
                  if (nameCtrl.text.isEmpty) {
                    _showSnack('Name cannot be empty', AppTheme.negative); return;
                  }
                  Navigator.pop(ctx);
                  try {
                    await ApiClient.updateWallet(wallet['id'].toString(),
  ...
  void _confirmDelete(Map<String, dynamic> wallet) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      title: const Text('Delete Wallet', style: TextStyle(
          color: AppTheme.textPrimary, fontSize: 17, fontWeight: FontWeight.w700)),
      content: Text('Are you sure you want to delete "${wallet['name']}"? This cannot be undone.',
  ...
  // ── Binance live account card ──────────────────────────────────────────────
  Widget _binanceLiveCard() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        const Text('Binance Live Account', style: TextStyle(
            color: AppTheme.textPrimary, fontSize: 16, fontWeight: FontWeight.w600)),
        const Spacer(),
        Text('\$${_fmt(total.toString())}', style: const TextStyle(
            color: Color(0xFFF0B90B), fontSize: 14, fontWeight: FontWeight.w600)),
      ]),
      const SizedBox(height: 12),
      GlassCard(borderColor: const Color(0xFFF0B90B).withOpacity(0.2), child: Column(children: [
        Row(children: [
          Container(width: 32, height: 32,
              decoration: BoxDecoration(color: const Color(0xFFF0B90B).withOpacity(0.15),
                  borderRadius: BorderRadius.circular(8)),
              child: const Center(child: Text('B', style: TextStyle(
                  color: Color(0xFFF0B90B), fontWeight: FontWeight.w700, fontSize: 16)))),
          const SizedBox(width: 10),
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Binance Corporate Account', style: TextStyle(
                color: AppTheme.textPrimary, fontSize: 14, fontWeight: FontWeight.w600)),
            Text('Spot + Funding + Futures + Margin',
                style: TextStyle(color: AppTheme.textTertiary, fontSize: 10)),
          ])),
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              decoration: BoxDecoration(color: AppTheme.positive.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(6)),
              child: const Text('Live', style: TextStyle(
                  color: AppTheme.positive, fontSize: 10, fontWeight: FontWeight.w600))),
        ]),
        const SizedBox(height: 12),
        Align(alignment: Alignment.centerLeft,
            child: Text('\$${_fmt(total.toString())} USD', style: const TextStyle(
        ...
        ...walletTypes.map((wt) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 10),
              child: Row(children: [
                ...
                const SizedBox(width: 10),
                Expanded(child: Text(wt['label'] as String,
                    style: const TextStyle(color: AppTheme.textSecondary, fontSize: 13))),
                Text('\$${_fmt(wdTotal.toString())}', style: const TextStyle(
                    color: AppTheme.textPrimary, fontSize: 13, fontWeight: FontWeight.w600)),
              ]));
        }),
        if (topAssets.isNotEmpty) ...[
          ...
          const Align(alignment: Alignment.centerLeft,
              child: Text('Top Assets', style: TextStyle(
          ...
          ...topAssets.take(5).map((a) {
            ...
            return Padding(padding: const EdgeInsets.only(bottom: 10),
                child: Row(children: [
                  ...
                  const SizedBox(width: 10),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(a['asset'] ?? '', style: const TextStyle(
                  ...
                  const SizedBox(width: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    Text('\$${_fmt(usd.toString())}', style: const TextStyle(
  ...
  String _fmtBalance(double n) {
    if (n == 0) return '0.00';
    if (n < 0.0001) return n.toStringAsFixed(8);
