...
class PropertyPortfolioScreen extends StatefulWidget {
  ...
  @override
  State<PropertyPortfolioScreen> createState() => _PropertyPortfolioScreenState();
}
...
class _PropertyPortfolioScreenState extends State<PropertyPortfolioScreen> {
  ...
  // ROI trend data — Target vs Actual
  final List<double> _targetRoi = [8.0, 8.0, 8.0, 8.0, 8.0, 8.0];
  final List<double> _actualRoi = [7.2, 7.8, 8.1, 7.9, 8.2, 7.9];

  // Cash flow by market
  final Map<String, double> _cashFlowByMarket = {
    'Austin': 4000,
    'Dallas': 3000,
    'Houston': 11500,
  };
  ...
  double get _totalValue =>
      _properties.fold(0, (sum, p) => sum + (p['value'] as num));
  double get _avgRoi =>
      _properties.fold(0.0, (sum, p) => sum + (p['roi'] as num)) /
      _properties.length;
  double get _totalCashFlow =>
      _properties.fold(0, (sum, p) => sum + (p['cashflow'] as num));
    ...
    child: Container(
      ...
      child: Row(children: [
        Container(
          width: 32, height: 32,
          decoration: BoxDecoration(color: _green.withOpacity(0.2), shape: BoxShape.circle),
          child: const Icon(Icons.person_outline, color: _greenLight, size: 16)),
        const SizedBox(width: 10),
        const Expanded(
            ...
            const Expanded(
                child: Text('YOUR REAL ESTATE PORTFOLIO',
                    style: TextStyle(
                        ...
                        letterSpacing: 0.5))),
            Icon(Icons.settings_outlined, color: Colors.white.withOpacity(0.6), size: 18),
          ]),
        ),
        // Stats row
        Padding(
          ...
          child: Row(children: [
            _statBox('TOTAL VALUE', '\$${_fmtValue(_totalValue)}', Icons.trending_up,
                _actualRoi, Colors.teal),
            _divider(),
            _statBox('ANNUAL ROI', '${_avgRoi.toStringAsFixed(1)}%',
                Icons.check_circle_outline, _actualRoi, _greenLight,
                subtitle: 'Target 8%'),
            _divider(),
            _statBox('NET CASH FLOW', '\$${_fmtCash(_totalCashFlow)}/mo',
                Icons.account_balance_wallet_outlined, _cashFlowByMarket.values.toList(),
  ...
  Widget _statBox(String label, String value, IconData icon, List<double> sparkData,
      Color color, {String? subtitle}) {
    return Expanded(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label,
            style: TextStyle(
                color: Colors.white.withOpacity(0.5), fontSize: 9, letterSpacing: 0.5)),
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Container(
          width: 44, height: 44,
          decoration: BoxDecoration(
              color: _green.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
          child: Icon(
              p['type'] == 'Multi-Family' ? Icons.apartment_outlined
                  : p['type'] == 'Condo' ? Icons.business_outlined
                  : Icons.home_outlined,
              color: _greenLight, size: 22)),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Text('Current ROI ${p['roi']}%',
                style: TextStyle(
                    color: isUp ? _greenLight : Colors.red.shade300, fontSize: 11)),
        ...
        Container(
          ...
          decoration: BoxDecoration(
              color: isUp ? _green.withOpacity(0.2) : Colors.red.shade900.withOpacity(0.3),
          ...
          child: SizedBox(height: 70,
              child: CustomPaint(
                  ...
                  painter: _DualLinePainter(
                      target: _targetRoi, actual: _actualRoi,
                      targetColor: Colors.white30, actualColor: _greenLight))),
          legend: Row(children: [
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Text(source, style: const TextStyle(color: Colors.white24, fontSize: 9)),
        ]),
    ...
    child: Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Expanded(child: Text(
          ...
          'This is primarily due to a temporary increase in maintenance costs for the multi-family property. '
          'Cash flow remains strong, but let\'s review your cost-optimization strategies. '
          'We should also consider expanding into the emerging San Antonio market, '
          'which matches your gentrifying criteria and high-growth trend.',
...
class _SparklinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double y(double v) => size.height - ((v - min) / range) * size.height * 0.8 - size.height * 0.1;
...
class _DualLinePainter extends CustomPainter {
  ...
  const _DualLinePainter(
      {required this.target, required this.actual,
        required this.targetColor, required this.actualColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double y(double v) =>
        size.height - ((v - min) / range) * size.height * 0.8 - size.height * 0.1;
    ...
    for (final entry in [(target, targetColor), (actual, actualColor)]) {
      ...
      for (int i = 1; i < vals.length; i++) {
        path.cubicTo(
            (x(i - 1, vals.length) + x(i, vals.length)) / 2, y(vals[i - 1]),
            (x(i - 1, vals.length) + x(i, vals.length)) / 2, y(vals[i]),
            x(i, vals.length), y(vals[i]));
      }
      canvas.drawPath(path,
          Paint()..color = color..strokeWidth = 1.5..style = PaintingStyle.stroke
              ..strokeCap = StrokeCap.round);
